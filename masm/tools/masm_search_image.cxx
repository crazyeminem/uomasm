...
//:
// \file
// \brief Tool to run a single ASM on one or more images.
// \author Tim Cootes
...
void print_usage()
{
  vcl_cout << "masm_search_image -p param_file\n"
           << "Tool to run a single ASM on one or more images.\n"
           << vcl_endl;
...
int main(int argc, char** argv)
{
  ...
  for (unsigned i=0;i<n_images;++i)
  {
    ...
    asm_searcher.set_to_mean();
    asm_searcher.set_pose(model.shape_model().default_pose());
    ...
    unsigned n_its=10;
    for (unsigned j=0;j<n_its;++j)
    {
      msm_points points0=asm_searcher.points();
      asm_searcher.update_step(image_pyr);

      if (params.out_image_width>0)
      {
        vcl_stringstream out_path;
        out_path<<params.output_dir<<"/"<<im_name<<"_asm";
        if (j<10) out_path<<"0";
        out_path<<j<<".eps";
        ...
        msm_draw_shape_to_eps(writer,points0,curves);
        ...
        writer.close();
        if (j==0) 
          vcl_cout<<"First frame save to : "<<out_path.str()<<vcl_endl;
      }
    }
