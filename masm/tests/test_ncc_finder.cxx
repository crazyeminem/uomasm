...
*/
// This is mul/mfpf/tests/test_ncc.cxx
...
void test_ncc_search(masm_point_finder_builder& b)
{
  ...
  TEST_NEAR("Evaluate at true point",pf->fit_at_point(image,mfpf_pose(p0,u)),0.0,1e-6);

  double fit;
  pf->set_search_area(0,0);
  pf->search(image,mfpf_pose(p0,u),new_p,fit);
  vcl_cout<<"Found point: "<<new_p<<vcl_endl;
  TEST_NEAR("Search with one point",fit,0.0,1e-6);
...
void test_ncc_search2()
{
  masm_ncc_finder_builder ncc_builder;
  masm_ncc_finder_model ncc_model;
  ncc_builder.set_kernel_size(5,5);
  ...
  ncc_builder.clear(1);
  ncc_builder.add_sample(image,mfpf_pose(p0,u));
  ncc_builder.build(ncc_model);

  vcl_cout<<"Built model: "<<ncc_model<<vcl_endl;
  
  masm_point_finder* pf = ncc_model.create_finder();
...
void test_ncc_finder()
{
  vcl_cout << "**************************\n"
           << " Testing masm_ncc_finder\n"
           << "**************************\n";

  masm_bldr_add_all_loaders();

  masm_ncc_finder_builder ncc_builder;
  ncc_builder.set_kernel_size(4,4);
  test_ncc_search(ncc_builder);
  ...
  // -------------------------------------------
  {
    ...
    TEST("Correct Point Finder Builder", pf->is_a(),"masm_ncc_finder_builder");
    if (pf->is_a()=="masm_ncc_finder_builder")
    {
      masm_ncc_finder_builder &a_pf = static_cast<masm_ncc_finder_builder&>(*pf);
...
//      TEST("nj configured",a_pf.nj(),5);
...

  {
    ...
    masm_ncc_finder_builder b;
    masm_point_finder_model* pf_model = b.new_finder();
    TEST("Builder: Correct Finder Model",pf_model->is_a(),"masm_ncc_finder_model");
    delete pf_model;
  }

  {
    ...
    kernel.fill(1.23);
    ncc_model.set(kernel,vgl_point_2d<double>(1.5,2.5));

    // Test binary load and save
    masm_point_finder_model * base_ptr = &ncc_model;

    vsl_b_ofstream bfs_out("test_ncc.bvl.tmp");
    TEST ("Created test_ncc.bvl.tmp for writing", (!bfs_out), false);
    vsl_b_write(bfs_out, ncc_model);
    vsl_b_write(bfs_out, base_ptr);
    bfs_out.close();

    masm_ncc_finder_model ncc_in;
    masm_point_finder_model *base_ptr_in = 0;

    vsl_b_ifstream bfs_in("test_ncc.bvl.tmp");
    TEST ("Opened test_ncc.bvl.tmp for reading", (!bfs_in), false);
    vsl_b_read(bfs_in, ncc_in);
    vsl_b_read(bfs_in, base_ptr_in);
    TEST ("Finished reading file successfully", (!bfs_in), false);
    bfs_in.close();
    vcl_cout<<ncc_model<<vcl_endl
            <<ncc_in<<vcl_endl;
//    TEST("Loaded==Saved",ncc_in,ncc_model);
    TEST("Load ncc by base ptr (type)",
         base_ptr_in->is_a()==ncc_model.is_a(),true);
