...
masm_lin_reg_model::masm_lin_reg_model()
...
void masm_lin_reg_model::set(const vcl_vector<mbl_chord>& roi,
                          unsigned n_planes,
...
//  Units of pose.u()
void masm_lin_reg_model::set_search_area(unsigned ni, unsigned nj)
{
  search_ni_=ni;
  search_nj_=nj;
}
...
masm_point_finder* masm_lin_reg_model::create_finder() const
{
  masm_lin_reg_finder* finder = new masm_lin_reg_finder();
  finder->set_model(*this);
  return finder;
}

//: Return prediction based on normalised sample vector
vgl_vector_2d<double> masm_lin_reg_model::offset_prediction(const vnl_vector<double>& norm_sample) const
{
  double dx = dot_product(norm_sample,rx_)+rx0_; 
  double dy = dot_product(norm_sample,ry_)+ry0_;
...
void masm_lin_reg_model::get_outline(vcl_vector<vgl_point_2d<double> >& pts) const
{
  pts.resize(7);
...
vcl_string masm_lin_reg_model::is_a() const
{
  return vcl_string("masm_lin_reg_model");
}


//: Create a copy on the heap and return base class pointer
masm_point_finder_model* masm_lin_reg_model::clone() const
{
  return new masm_lin_reg_model(*this);
}
...
void masm_lin_reg_model::print_summary(vcl_ostream& os) const
{
...
void masm_lin_reg_model::b_write(vsl_b_ostream& bfs) const
{
  ...
  vsl_b_write(bfs,rx_);
  vsl_b_write(bfs,rx0_);
  vsl_b_write(bfs,ry_);
  vsl_b_write(bfs,ry0_);
...
void masm_lin_reg_model::b_read(vsl_b_istream& bfs)
{
  if (!bfs) return;
  short version;
